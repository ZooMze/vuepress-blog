...
bind(el, binding, vNode) {
  ...
  const replaceTagsLength = vnode.context.replaceTags.length
  if(!binding.value || (redTagsLength == 0 && replaceTagsLength == 0)) {
...
```js
[
  '<span style="color: #fff;">这</span>',
  '<span style="color: #fff;">是</span>',
  '<span style="color: #fff;">数</span>',
  '<span style="color: #fff;">据</span>'
]
```
...
resultArray = valueArray.map((value, valueIndex) => {
  ...
  return flag ? value.replace('#fff' , 'red') : value // 替换颜色
})
```
